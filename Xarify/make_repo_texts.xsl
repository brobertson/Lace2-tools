...
<xsl:stylesheet xmlns:repo="http://exist-db.org/xquery/repo"   xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="*">
    ...
    <repo:meta>
	    <repo:description>OCR run <xsl:value-of select="$accuracy"/> on <xsl:value-of select="$rundate"/> of <xsl:value-of select="creator[1]"/> (<xsl:value-of select="date"/>)  <xsl:value-of select="title"/><xsl:apply-templates select="volume"/></repo:description>
